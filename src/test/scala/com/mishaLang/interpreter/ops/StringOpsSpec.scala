...
class StringOpsSpec extends BaseInterpreterSpec {
	...
	it should "correctly cast values to strings" in {
		...
		assert(StringOps.castToString(Value.Number(50, Percentage)).get === Value.String("50%"))
		assert(StringOps.castToString(Value.Number(123, Atomic(Pixel))).get === Value.String("123px"))
		assert(StringOps.castToString(Value.Number(0, Atomic(Pixel))).get === Value.String("0"))
		assert(StringOps.castToString(Value.Number(0, Atomic(Radian))).get === Value.String("0rad"))
	...
	it should "correctly multiply strings" in {
		assert(StringOps.multiply(Value.String("a"), Number(4)) === Value.String("aaaa"))
		assert(StringOps.multiply(Value.String("a"), Number(0)) === Value.String(""))
		assert(StringOps.multiply(Value.String("a"), Number(-123)) === Value.String(""))
	}
}
